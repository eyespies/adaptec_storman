...
describe 'adaptec_storman::manager' do
    ...
    versions.each do |version|
      ...
      cpus.each do |cpu|
        context "On #{platform} #{version} with a #{cpu} CPU" do
          ...
          if details['source_files']['chefspec'].key?('manager') && details['source_files']['chefspec']['manager'].key?(source_key)
            ...
            source_file = details['source_files']['chefspec']['manager'][source_key]
            if source_file =~ /\.(deb|rpm)$/
              ...
              it 'should output an error message when an installation package is NOT available' do
                  expect(chef_run).to write_log('Installation of the manager files via standard binary executable is not yet supported')
              end
            end
          else
